---
title: "Get Started with DeconvoBuddies"
author: 
...
vignette: >
  %\VignetteIndexEntry{Get Started with DeconvoBuddies}
...
This package is associated with "Benchmark of cellular deconvolution methods 
using a multi-assay reference dataset from postmortem human prefrontal cortex" 
from Huuki-Myers et al.
(10.1101/2024.02.09.579665v2)[https://www.biorxiv.org/content/10.1101/2024.02.09.579665v2].
...
There are three palettes to choose from to generate colors or users can provide 
their own color palette:

-   "classic" (default): classic set of 8 cell type colors from LIBD, checked 
for visability and color blind accessibility.
...
The `r Biocpkg("DeconvoBuddies")` package `r Citep(bib[["DeconvoBuddies"]])` was
...
* `r Biocpkg("graphics")` `r Citep(bib[["graphics"]])`
* `r Biocpkg("grDevices")` `r Citep(bib[["grDevices"]])`
* `r Biocpkg("matrixStats")` `r Citep(bib[["matrixStats"]])`
* `r Biocpkg("methods")` `r Citep(bib[["methods"]])`
* `r Biocpkg("purrr")` `r Citep(bib[["purrr"]])`
* `r Biocpkg("rafalib")` `r Citep(bib[["rafalib"]])`
* `r Biocpkg("reshape2")` `r Citep(bib[["reshape2"]])`
* `r Biocpkg("S4Vectors")` `r Citep(bib[["S4Vectors"]])`
* `r Biocpkg("scran")` `r Citep(bib[["scran"]])`
* `r Biocpkg("SingleCellExperiment")` `r Citep(bib[["SingleCellExperiment"]])`
* `r Biocpkg("spatialLIBD")` `r Citep(bib[["spatialLIBD"]])`
* `r Biocpkg("stats")` `r Citep(bib[["stats"]])`
* `r Biocpkg("stringr")` `r Citep(bib[["stringr"]])`
* `r Biocpkg("SummarizedExperiment")` `r Citep(bib[["SummarizedExperiment"]])`
* `r Biocpkg("tibble")` `r Citep(bib[["tibble"]])`
* `r Biocpkg("utils")` `r Citep(bib[["utils"]])`
