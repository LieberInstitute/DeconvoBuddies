...
The columns of this table are documented in detail at `DeconvoBuddies::get_mean_ratio()`. Though `cellType.target` lists the target cell type for which the `Mean Ratio` is being calculated. It is the numerator of the `Mean Ratio`. The `cellType.2nd` lists the cell type against which the target cell type is being compared. It is the denominator of the `Mean Ratio`.
...
Note how `plot_marker_express()` lists the 2 cell types that are being compared that result in the specific numerical `Mean Ratio` value being displayed. As we specified that our target cell type is `"Excit"` in our call to `plot_marker_express()`, the numerator is `Excit` in all the panels shown above.
